...
<%!
    ...
    private boolean passHeadersInfo(Map<String,String> mapHeaderInfo, HttpURLConnection con) {
    	for (Iterator<Map.Entry<String, String>> entries = mapHeaderInfo.entrySet().iterator(); entries.hasNext(); ) {
    	    Map.Entry<String, String> pair = entries.next();
    	    con.setRequestProperty(pair.getKey(),pair.getValue());
    	    mapHeaderInfo.entrySet().iterator().remove(); // avoids a ConcurrentModificationException
    	}
        return true;
    }
    ...
    private HttpURLConnection doHTTPRequest(String uri, byte[] bytes, String method, Map<String,String> mapHeaderInfo) throws IOException{
        URL url = new URL(uri);
